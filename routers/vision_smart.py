...
async def vision_smart_status():
    return {
        ...
        "endpoints": ["/vision/analyze-smart", "/vision/analyze-smart-path", "/vision-smart/status"],
        "policy": {"cpu_ram_only": True, "gpu": "untouched", "no_ads": True, "no_powershell_unless_technical_screenshot": True},
    }
...
async def analyze_smart_upload(file: UploadFile = File(...), prompt: str = Form("")):
