...
_decision_router = DecisionRouter([SearxngProvider(), JinaProvider(), TavilyProvider(), GeminiProvider(), OllamaProvider()])
...
async def search_endpoint(payload: ResearchRequest):
    try:
        ...
        result = await _decision_router.search(query=payload.query, mode=search_mode, max_tokens=payload.max_tokens or 400)

        provider_name = result.get("provider", "unknown")
        data_content = result.get("data", {})

        # Archivage transactionnel de la preuve
        await _evidence_store.store(query=payload.query, provider=provider_name, mode=payload.mode, data=data_content, task_id=task_id)
