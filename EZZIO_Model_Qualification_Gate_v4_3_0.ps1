...
function Test-Model {
    ...
    $robustnessScore  = Get-CategoryScore $allResults 'Robustness'
    ...
    # CAPABILITY
    # ---------------------------------------------------------------------
    # BUG v4.3.0 corrigé : le test ROB-01 (catégorie Robustness) était exécuté
    # et noté par Evaluate-Response, mais son score n'était jamais agrégé
    # (absent de Get-CategoryScore, absent de $capabilityScore, absent du
    # profil exporté) — violation directe du principe 6 du script
    # ("Aucun résultat silencieusement ignoré"). RobustnessScore est
    # maintenant calculé et intégré, avec rééquilibrage des poids (somme
    # toujours = 1.00) : General 0.15 -> 0.05, Robustness ajouté à 0.10.
...
Write-Forensic -Message "RunId=$RunId" -Level INFO
Write-Forensic -Message "Root=$Root" -Level INFO
Write-Forensic -Message "CPU=$CpuName" -Level INFO
Write-Forensic -Message "Threads=$CpuThreads" -Level INFO
Write-Forensic -Message "RAM=${TotalRamGB}GB" -Level INFO
Write-Forensic -Message "MODE=CPU-ONLY" -Level INFO
...
Write-Forensic -Message "$($models.Count) modèle(s) à qualifier." -Level PASS
...
if ($initialRunning.Count -gt 1) {
    Write-Forensic `
        -Message ("FAIL-CLOSED : plusieurs modèles sont déjà actifs : " + ($initialRunning -join ', ')) `
        -Level FAIL
    throw "INITIAL_MULTIPLE_MODELS"
}

if ($initialRunning.Count -eq 1) {
    Write-Forensic -Message "Un modèle est déjà actif : $($initialRunning[0])" -Level WARN
...
Write-Forensic -Message "Etat initial conforme : zéro modèle actif." -Level PASS
...
foreach ($model in $models) {
    ...
    Write-Host ''
    Write-Host (">>> MODELE $modelIndex/$totalModels : $($model.Name)") -ForegroundColor Cyan
    ...
    try {
        ...
        Write-Forensic `
            -Message ("Qualification interrompue pour $($model.Name) : " + $_.Exception.Message) `
            -Level FAIL
    ...
    if ($active.Count -gt 1) {
        Write-Forensic -Message "VIOLATION SINGLE-MODEL détectée après test." -Level FAIL
    ...
    if ($active.Count -eq 1) {
        ...
        if (-not $released) {
            Write-Forensic -Message "Impossible de libérer $($active[0]) après qualification." -Level FAIL
            throw "POST_MODEL_UNLOAD_FAILURE"
        }
    }

    Write-Forensic -Message "Modèle $($model.Name) complètement sorti du runtime." -Level PASS
}
...
foreach ($profile in $ranking) {
    ...
    Write-Host ''
    Write-Host ("#{0}  {1}" -f $rank, $profile.Model) -ForegroundColor White
    Write-Host ("     OVERALL     : {0}/100" -f $profile.OverallScore)
    Write-Host ("     CAPABILITY  : {0}/100" -f $profile.CapabilityScore)
    Write-Host ("     RELIABILITY : {0}/100" -f $profile.ReliabilityScore)
    Write-Host ("     PERFORMANCE : {0}/100" -f $profile.PerformanceScore)
    Write-Host ("     REASONING   : {0}/100" -f $profile.ReasoningScore)
    Write-Host ("     CODE        : {0}/100" -f $profile.CodeScore)
    Write-Host ("     FRENCH      : {0}/100" -f $profile.FrenchScore)
    Write-Host ("     STRUCTURED  : {0}/100" -f $profile.StructuredScore)
    Write-Host ("     ROBUSTNESS  : {0}/100" -f $profile.RobustnessScore)
    Write-Host ("     AVG TPS     : {0}" -f $profile.AvgTokensPerSec)
    Write-Host ("     FAILURE     : {0}%" -f $profile.FailureRate)
    Write-Host ("     VERDICT     : {0}" -f $profile.Verdict)
}
...
if ($finalRunning.Count -gt 1) {
    Write-Forensic `
        -Message ("FAIL-CLOSED FINAL : $($finalRunning.Count) modèles encore actifs : " + ($finalRunning -join ', ')) `
        -Level FAIL
    throw "FINAL_MULTIPLE_MODELS"
}

if ($finalRunning.Count -eq 1) {
    Write-Forensic -Message "Libération finale : $($finalRunning[0])" -Level SWITCH
    $released = Unload-OllamaModel -Model $finalRunning[0]
    if (-not $released) {
        Write-Forensic -Message "Impossible de confirmer la libération finale." -Level FAIL
...
if ($finalCheck.Count -ne 0) {
    Write-Forensic -Message "FAIL-CLOSED : un modèle reste actif après nettoyage." -Level FAIL
...
$qualifiedCount = @($profiles | Where-Object { $_.Verdict -eq 'QUALIFIED' }).Count
$partialCount = @($profiles | Where-Object { $_.Verdict -eq 'PARTIAL' }).Count
$failedCount = @($profiles | Where-Object { $_.Verdict -eq 'FAIL' }).Count
...
if ($null -ne $best) {
    Write-Host ("BEST MODEL      : {0}" -f $best.Model) -ForegroundColor Cyan
    Write-Host ("BEST OVERALL    : {0}/100" -f $best.OverallScore) -ForegroundColor Cyan
}
