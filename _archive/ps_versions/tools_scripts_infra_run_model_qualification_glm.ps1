...
$AuditRoot = Join-Path $Root 'audit\model_gate_glm_dedicated'
...
function Test-Model {
    ...
    foreach ($test in $Tests) {
        $r = Invoke-ModelTest `
            ...
            -Prompt $test.Prompt `
            -MaxTokens ([math]::Max($test.MaxTokens * 4, 1536)) `
...
$TargetModelNames = @('glm-4.7-flash:Q4_K_M')
$models = @($genList | Where-Object { $TargetModelNames -contains $_.Name })
