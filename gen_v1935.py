...
print("[*] E-ZZIO v1.9.3.5 - Génération propre des exécuteurs via raw strings...")

ps_content = r"""import subprocess
import time
...
class PowerShellExecutor(ExternalExecutorBase):
    ...
    def execute(context, *args, **kwargs):
        ...
        if not allowed or "Remove-Item" in cmd_str or r"C:\" in cmd_str:
...
fs_content = r"""from pathlib import Path
from runtime.external.base import ExternalExecutorBase
from runtime.tools.tool_schema import ToolResult
...
Path("runtime/tools/executors/powershell.py").write_text(ps_content, encoding="utf-8")
Path("runtime/tools/executors/filesystem.py").write_text(fs_content, encoding="utf-8")
print("[+] Fichiers écrits proprement.")
