...
# ==========================================
# 1. JSON LOGGING & THREAD EXCEPTHOOK
# ==========================================
...
def setup_logger(log_path):
    ...
    logger = logging.getLogger("IndexEngineV52")
...
class TelemetryCollector:
    ...
    def report(self, logger):
        ...
        logger.info(f"Report Final V5.2 : {speed} fichiers/sec sur {round(total_time, 2)}s", extra={"extra_data": metrics})
    ...
    def migrate(conn, logger):
        ...
        for col, col_type in required_cols.items():
            if col not in existing_cols:
                logger.info(f"Migration BDD : Ajout de la colonne '{col}' ({col_type})...")
...
class SQLiteRepository:
    ...
    def init_schema(self):
        with self.get_connection(read_only=False) as conn:
            ...
            END;
            """)
            cursor.execute("INSERT OR REPLACE INTO schema_metadata (key, value) VALUES ('version', '5.2')")
            conn.commit()
...
class HasherWorker:
    """Worker avec connexion SQLite 100% locale créée au sein de son propre thread."""
    ...
    def run(self):
        # La connexion est instanciée ICI, à l'intérieur du thread worker !
        ...
        try:
            ...
            self.telemetry.inc_error()
            # En cas de crash inattendu, on envoie quand même la sentinelle pour éviter tout Deadlock du Writer !
            self.out_queue.put("SCAN_DONE")
            raise e
        finally:
            conn_read.close()
...
# ==========================================
# 6. ORCHESTRATEUR V5.2
# ==========================================
class WorkspaceIndexerV52:
    def __init__(self, config_path="config/indexer_config.json"):
        ...
        # Calibrage dynamique pour Ryzen 9 5900X (Plafond optimal à 12 workers)
    ...
    def run(self):
        self.logger.info(f"Démarrage Moteur V5.2 ({self.num_workers} Hash Workers, BLAKE3={HAS_BLAKE3})")
        ...
        # 2. Workers Threads (Connexion locale dans le run)
        worker_threads = []
