...
def setup_logger(log_path):
    ...
    logger = logging.getLogger("IndexEngineV53")
...
class TelemetryCollector:
    ...
    def report(self, logger):
        ...
        logger.info(f"Report Final V5.3 : {speed} fichiers/sec sur {round(total_time, 2)}s", extra={"extra_data": metrics})

# Référence globale pour le hook d'exception des threads
...
class MigrationManager:
    ...
    def migrate(conn, logger):
        ...
        # 2. Migration & Auto-Réparation FTS5
        ...
        if fts_exists:
            ...
            if not required_fts.issubset(fts_cols):
                logger.warning("Structure FTS5 obsolète détectée. Reconstruction automatique en cours...")
                rebuild_fts = True
        else:
            rebuild_fts = True
        ...
        if rebuild_fts:
            ...
            logger.info("Index FTS5 reconstruit et synchronisé avec succès.")
...
class SQLiteRepository:
    ...
    def init_schema(self):
        with self.get_connection(read_only=False) as conn:
            ...
            cursor.execute("INSERT OR REPLACE INTO schema_metadata (key, value) VALUES ('version', '5.3')")
            conn.commit()
                        ...
                        for entry in it:
                            try:
                                ...
                                if entry.is_dir(follow_symlinks=False):
                                    if entry.name not in self.excluded_dirs: dirs.append(entry.path)
...
# ==========================================
# 6. ORCHESTRATEUR V5.3
# ==========================================
class WorkspaceIndexerV53:
    def __init__(self, config_path="config/indexer_config.json"):
        ...
        # Injection de la télémétrie dans le hook global de threads
    ...
    def run(self):
        self.logger.info(f"Démarrage Moteur V5.3 ({self.num_workers} Hash Workers, BLAKE3={HAS_BLAKE3})")
