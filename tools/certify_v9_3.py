...
print("=" * 70)
print("   E-ZZIO V9.3 — 100% VISUAL REVEAL & PRODUCT UX CERTIFIER")
print("=" * 70)
...
# 5. Full Certified Test Suite (128 tests)
print("[5/10] Execution de la suite de certification complete V9.3 (128 tests)...")
test_cmd = [
...
if pytest_res.returncode != 0:
    ...
    sys.exit(1)
print("[OK] 128/128 tests PASS (100% hygiene verte).")
...
print("[7/10] Verification des preuves visuelles (8 screenshots certifiés)...")
vis_dir = ROOT / "state/audit/visual/v9.3/final"
expected_shots = [
    "01-command-center.png", "02-office-overview.png", "03-agent-inspector.png",
    "04-task-view.png", "05-hitl-view.png", "06-provider-view.png",
...
for s in expected_shots:
    ...
    with open(p, "rb") as f:
        if f.read(8) != b"\x89PNG\r\n\x1a\n":
            ...
            sys.exit(1)
print(f"[OK] 8/8 captures visuelles certifiees presentes et valides ({len(expected_shots)}/{len(expected_shots)}).")

# 8. Documentation Suite Verification
print("[8/10] Verification de la suite documentaire V9.3...")
doc_files = [
    "docs/V9.3_VISUAL_AUDIT.md",
    "docs/V9.3_DESIGN_SYSTEM.md",
    "docs/V9.3_VISUAL_RELEASE_REPORT.md",
    "docs/USER_GUIDE.md",
    "docs/V9.3_GOLDEN_COMPARISON.md"
]
for d in doc_files:
    if not (ROOT / d).exists():
        print(f"[FAIL] Document {d} manquant !")
        sys.exit(1)
print("[OK] 5/5 documents V9.3 presents et valides.")

# 9. Packaging Golden Release Source
print("[9/10] Packaging du Golden Source Archive (E-ZZIO-V9.3-GOLDEN-SOURCE.zip)...")
res_build = subprocess.run([sys.executable, "tools/build_golden_archive_v9_3.py"], capture_output=True, text=True)
if res_build.returncode != 0:
    print(f"[FAIL] Echec du packaging: {res_build.stdout} {res_build.stderr}")
    sys.exit(1)
print("[OK] Golden Source Archive & Manifest V9.3 generes avec succes.")

# 10. Verification of Manifest & Seal
print("[10/10] Verification du Manifeste de Provenance V9.3...")
manifest_path = ROOT / "dist/releases/E-ZZIO-V9.3-GOLDEN-MANIFEST.json"
...
print("=" * 70)
print("   E-ZZIO V9.3 CERTIFICATION 100% COMPLETE & SCELLEE AVEC SUCCES")
print("=" * 70)
