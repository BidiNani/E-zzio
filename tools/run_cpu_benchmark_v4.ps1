...
# ============================================================
# E-ZZIO - CPU MODEL BENCHMARK V4 (CORRECTIF CERTIFIE)
...
# 8 modeles generatifs (hors modeles de plongement vectoriel)
$Models = @(
    "phi4-mini:latest",
    "qwen3:8b",
...
$JsonReport    = Join-Path $ReportDir "cpu_model_benchmark_v4_$Stamp.json"
$CsvReport     = Join-Path $ReportDir "cpu_model_benchmark_v4_$Stamp.csv"
$ProfileReport = Join-Path $ReportDir "cpu_model_profiles_v4_$Stamp.json"

Write-Host "============================================================"
Write-Host " E-ZZIO - CPU MODEL BENCHMARK V4 (8 MODELES GENERATIFS)"
Write-Host "============================================================"
...
Write-Host "Quality    : Floor strict >= 80 (Moyenne ET chaque sous-tache)"
Write-Host "============================================================"
Write-Host ""

# ============================================================
# HELPER DE SERIALISATION IMMUABLE
# ============================================================
...
# ============================================================
# MOTEUR D'INFERENCE OLLAMA (API NATIVE)
# ============================================================
...
function Get-CompetenceScore {
    ...
    switch ($Benchmark) {
        ...
        "CODE" {
            $score = 0.0
            if ($text -match '(?m)^\s*def\s+\w+\s*\(') { $score += 30.0 }
        ...
        "JSON" {
            try {
                ...
                if ($clean -notmatch '(?s)^\s*\{.*\}\s*$') { return 0.0 }
...
foreach ($model in $Models) {
    ...
    # 1. Competences de reference (12 threads / 4096 ctx)
        ...
        foreach ($depth in @(0.15, 0.50, 0.85)) {
            ...
            foreach ($decoy in @($false, $true)) {
                ...
                if ($result.Success) {
                    ...
                    if ($result.Content -match ("(?m)^\s*VALEUR=\s*" + $expected + "\s*$")) {
                        $score = 100.0
                    }
                }
    ...
    # Normalisation locale pour la selection intra-modele
    $localMaxTps = ($modelTuningRaw | Measure-Object -Property AverageTPS -Maximum).Maximum
    if ($null -eq $localMaxTps -or $localMaxTps -le 0) { $localMaxTps = 1.0 }

    foreach ($row in $modelTuningRaw) {
        $localSpeedNorm = [math]::Min(100.0, [double](($row.AverageTPS / $localMaxTps) * 100.0))
        $localCombined  = if (-not $row.QualityGate) { 0.0 } else {
            [math]::Min(100.0, [double](($row.AverageScore * 0.75) + ($localSpeedNorm * 0.25)))
        }
        ...
        $TuningResults.Add([pscustomobject]@{
            ...
            LocalSpeedNorm   = [math]::Round($localSpeedNorm, 2)
            LocalUtility     = [math]::Round($localCombined, 2)
            QualityGate      = $row.QualityGate
        })

        $status = if ($row.QualityGate) { "VALID" } else { "REJECT" }
        Write-Host (
            "    T={0,2} C={1,5} avg={2,6:N1} min={3,6:N1} TPS={4,6:N2} local_utility={5,6:N2} [{6}]" -f `
            $row.Threads, $row.Context, $row.AverageScore, $row.MinTaskScore, $row.AverageTPS, $localCombined, $status
        )
    }

    # 4. Selection deterministe & Confirmation
    ...
    if ($eligible.Count -eq 0) {
        ...
        $candidate = $eligible |
            Sort-Object `
                @{Expression = {$_.LocalUtility}; Descending = $true},
        ...
        if (-not $finalQualityGate) {
            Write-Host "  [REPLI] Confirmation echouee -> repli sur profil non confirme." -ForegroundColor Yellow
            ...
            if ($null -eq $fallback) {
                ...
                $RecommendedProfiles.Add([pscustomobject]@{
                    ...
                    Utility   = [pscustomobject]@{ AverageQuality = $fallback.AverageScore; MinTaskScore = $fallback.MinTaskScore; Instruction = $fallback.InstructionScore; Reasoning = $fallback.ReasoningScore; JSON = $fallback.JsonScore; AverageTPS = $fallback.AverageTPS; SpeedNorm = 0.0; CombinedUtility = 0.0; QualityGate = $fallback.QualityGate; ConfirmationPassed = $false }
                    Competencies = [pscustomobject]@{ GENERAL = $scores["GENERAL"]; REASONING = $scores["REASONING"]; CODE = $scores["CODE"]; JSON = $scores["JSON"]; INSTRUCTION = $scores["INSTRUCTION"] }
                    LongContext  = $niahByContext
                })

                Write-Host (
                    "[FALLBACK_UNCONFIRMED] {0} -> T={1} CTX={2}" -f `
                    $model, $fallback.Threads, $fallback.Context
            ...
            Write-Host (
                "[CERTIFIED] {0} -> T={1} CTX={2} avg={3:N1} min={4:N1} tok/s={5:N2}" -f `
                $model, $candidate.Threads, $candidate.Context, $finalAvgQuality, $finalMinTask, $finalAvgTps
            ) -ForegroundColor Green
        }
    }

    # Eviction memoire explicite (keep_alive = 0)
    Write-Host ""
    Write-Host "[UNLOAD] Dechargement du modele de la RAM..."
...
# ============================================================
# NORMALISATION GLOBALE INTER-MODELES & CLASSEMENT
# ============================================================

Write-Host ""
Write-Host "[5/5] Normalisation globale et export des rapports..."

$globalMaxTps = (@($RecommendedProfiles | Where-Object { $null -ne $_.Utility } | ForEach-Object { $_.Utility.AverageTPS }) | Measure-Object -Maximum).Maximum
if ($null -eq $globalMaxTps -or $globalMaxTps -le 0) { $globalMaxTps = 10.0 }

foreach ($p in $RecommendedProfiles) {
    if ($null -ne $p.Utility) {
        $speedNorm = [math]::Min(100.0, [double](($p.Utility.AverageTPS / $globalMaxTps) * 100.0))
        $combined  = [math]::Min(100.0, [double](($p.Utility.AverageQuality * 0.75) + ($speedNorm * 0.25)))
        $p.Utility.SpeedNorm       = [math]::Round($speedNorm, 2)
        $p.Utility.CombinedUtility = [math]::Round($combined, 2)
    }
}
...
$JsonPayload = [pscustomobject]@{
    ...
    BenchmarkPolicy = [pscustomobject]@{
        ...
        MinimumTaskScore   = 80.0
        MinimumAverage     = 80.0
        QualityWeight      = 0.75
        SpeedWeight        = 0.25
        GlobalMaxTPS       = [math]::Round($globalMaxTps, 4)
        GPU                = 0
        NIAHDepths         = @(0.15, 0.50, 0.85)
        NIAHDecoy          = $true
    }
...
Write-Host ""
Write-Host "============================================================"
Write-Host " BENCHMARK CPU V4 TERMINE (CLASSEMENT GLOBAL)"
Write-Host "============================================================"

if ($RankedProfiles.Count -eq 0) {
    Write-Host "[FAIL-CLOSED] Aucun profil CPU certifie." -ForegroundColor Red
}
else {
    $rank = 1
    foreach ($p in $RankedProfiles) {
        Write-Host ("#{0} {1}" -f $rank, $p.Model)
        Write-Host ("    global_utility={0:N1} quality={1:N1} min_task={2:N1} tok/s={3:N2}" -f $p.Utility.CombinedUtility, $p.Utility.AverageQuality, $p.Utility.MinTaskScore, $p.Utility.AverageTPS)
        Write-Host ("    threads={0} ctx={1} gpu={2} status={3}" -f $p.CPU.NumThread, $p.CPU.NumCtx, $p.CPU.NumGPU, $p.Status)
        $rank++
    }
}
