...
# -------------------------------------------------------------------------
# DÉTECTION MATÉRIELLE ET BANDE PASSANTE THÉORIQUE
# -------------------------------------------------------------------------
...
# =============================================================================
# [2/18] TÉLÉMÉTRIE MÉMOIRE & CONVERSIONS SÉCURISÉES
# =============================================================================
...
# =============================================================================
# [3/18] API OLLAMA & INVENTAIRE SÉCURISÉ
# =============================================================================
...
# =============================================================================
# [4/18] ISOLATION & DÉCHARGEMENT STRICT
# =============================================================================
...
# =============================================================================
# [6/18] COMMUTATION DE MODÈLE & TÉLÉMÉTRIE CHARGE RAM
# =============================================================================
...
function Switch-OllamaModel {
    ...
    Write-Forensic -Message ("RAM AVANT LOAD : {0} Go utilisés / {1} Go libres" -f $memBefore.UsedGB, $memBefore.FreeGB) -Level RAM
    ...
    Write-Forensic -Message ("RAM APRÈS LOAD : {0} Go utilisés (+{1} Go) | {2} Go libres" -f $memAfter.UsedGB, $deltaRAM, $memAfter.FreeGB) -Level RAM
...
# =============================================================================
# [7/18] EXTRACTION DES RÉPONSES ET BALISES <THINK>
# =============================================================================
...
# =============================================================================
# [8/18] ÉVALUATION DÉTERMINISTE DES RÉPONSES
# =============================================================================
...
function Evaluate-Response {
    ...
    $reasons.Add('three non-empty lines')
        ...
        'Reasoning' {
            if ($text -match '(?i)\d+') {
                $score += 15$reasons.Add('numeric reasoning evidence')
            }
    ...
    $reasons.Add('reasoning language')
        ...
        'Code' {
            if ($text -match '(?i)def\s+\w+\s*\(|function\s+\w+') {
                $score += 25$reasons.Add('function declaration')
            }
    ...
    $reasons.Add('control/return syntax')
            ...
            if ($text -match '(?i)null|none|\$null') {
                $score += 15$reasons.Add('null handling')
            }
            if ($text -match '(?i)```') {
                $score -= 10
...
# =============================================================================
# [9/18] EXÉCUTION D'UN TEST (DOUBLE MESURE TPS)
# =============================================================================
...
# =============================================================================
# [12/18] QUALIFICATION D'UN MODÈLE & TÉLÉMÉTRIE PHYSIQUE
# =============================================================================
...
Write-Forensic -Message ("RAM={0}GB DDR4-{1} MT/s ({2} canaux) | Bande passante théorique={3} Go/s" -f $TotalRamGB, $MemorySpeed, $MemoryChannels, $TheoreticalBandwidth) -Level INFO
...
Write-Host "MÉMOIRE VIVE            : $TotalRamGB Go DDR4-$MemorySpeed MT/s"
Write-Host "CANAUX MÉMOIRE          : $MemoryChannels ($MemorySticks barrettes)"
Write-Host "BANDE PASSANTE THÉORIQUE: $TheoreticalBandwidth Go/s"
Write-Host "RAM DISPONIBLE INITIALE : $AvailableRamGB Go"

# =============================================================================
# [14/18] DISPONIBILITÉ OLLAMA
# =============================================================================
...
foreach ($model in $models) {
    ...
    try {
        ...
        Write-Forensic -Message ("Modèle {0} complètement libéré. RAM disponible récupérée : {1} Go." -f $model.Name, $snapFinal.FreeGB) -Level RAM
    }
}
