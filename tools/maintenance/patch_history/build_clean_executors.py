...
print("[*] E-ZZIO v1.9.3.2 - Réécriture propre et robuste des exécuteurs...")

# 1. PowerShell Executor Propre
ps_path = Path("runtime/tools/executors/powershell.py")
ps_content = """import subprocess
import time
...
"""
ps_path.write_text(ps_content.strip(), encoding="utf-8")
print("[+] powershell.py réécrit proprement.")

# 2. FileSystem Executor Propre
fs_path = Path("runtime/tools/executors/filesystem.py")
fs_content = """from pathlib import Path
from runtime.external.base import ExternalExecutorBase
from runtime.tools.tool_schema import ToolResult
...
class FileSystemExecutor(ExternalExecutorBase):
    ...
    def execute(context, *args, **kwargs):
        ...
        try:
            ...
            return ToolResult(success=False, output="", error=str(e))
"""
fs_path.write_text(fs_content.strip(), encoding="utf-8")
print("[+] filesystem.py réécrit proprement.")
print("[OK] Exécuteurs stabilisés.")
