...
class IntelligenceRouter:
    ...
    def _log_immutable_transaction(self, intent: str, request_id: str, candidates: list, selected: str, state: str, execution_details: dict = None):
    ...
    async def route(self, task_intent: str, prompt: str, constraints: RouteConstraints) -> ExecutionDecision:
        ...
        if not cb_ollama_open:
            for model_name, meta in self.local_models.items():
                ...
                if task_intent in roles or (constraints.urgency == Urgency.REALTIME and "fast_chat" in roles) or (constraints.require_vision and "vision" in roles):
                    ...
                    score = RoutingScorer.calculate_final_score(
                        ...
                        health_status=1.0, reliability=0.90, latency_score=0.9 if state["is_loaded"] else 0.5, 
                        cost_score=1.0, confidence=1.0, constraints=constraints, 
                        is_local=True, is_loaded_in_ram=state["is_loaded"], high_ram_pressure=high_pressure
                    )
        ...
        if constraints.allow_cloud and not constraints.require_privacy:
            ...
            for prov_name, meta in cloud_caps.items():
                if not meta.get("enabled", False) or circuit_breaker.is_open(prov_name): continue
                roles = meta.get("roles", [])
                
                if task_intent in roles or (constraints.urgency == Urgency.REALTIME and "fast_chat" in roles) or (constraints.require_vision and "vision" in roles):
                    perf = cloud_perf.get(prov_name, {})
                    score = RoutingScorer.calculate_final_score(
                        capability_match=0.90, health_status=1.0, 
                        reliability=perf.get("effective_reliability", 0.5), 
                        latency_score=0.8, cost_score=0.9, confidence=perf.get("confidence", 0.0),
                        constraints=constraints, is_local=False
                    )
                    candidates.append({"plane": "cloud", "provider": prov_name, "model": meta.get("default_model", "auto"), "score": score})

        if not candidates:
            return ExecutionDecision(execution_plane="cloud", provider="groq", model="llama-3.3-70b-versatile", confidence_score=0.0, reason="default_fallback")
        ...
        decision = ExecutionDecision(
            execution_plane=winner["plane"], provider=winner["provider"], model=winner["model"],
            confidence_score=winner["score"], reason=f"Top score for intent '{task_intent}'",
            metadata={"request_id": request_id, "alternatives": alternatives, "candidates_log": candidates[:3], "ram_gb": winner.get("ram_gb", 0)}
        )

        # Immuable Ledger : État DECISION_MADE
        self._log_immutable_transaction(
            intent=task_intent, request_id=request_id,
            candidates=[{"provider": c["provider"], "model": c["model"], "score": c["score"]} for c in candidates[:3]],
            selected=winner["model"], state="DECISION_MADE",
    ...
    async def execute(self, decision: ExecutionDecision, prompt: str, image_bytes: bytes = None) -> ProviderResponse:
        ...
        self._log_immutable_transaction(
            intent=decision.reason, request_id=request_id,
            candidates=decision.metadata.get("candidates_log", []),
            selected=decision.model, state="EXECUTION_STARTED",
        ...
        try:
            ...
            response = await provider_instance.generate(prompt=prompt, model=decision.model, image_bytes=image_bytes, capability=decision.reason)
        ...
        if not response.ok:
            ...
            for alt in alternatives:
                ...
                print(f"[Fault Tolerant Core] Échec sur {decision.provider} -> Bascule sécurisée vers le circuit de secours {alt_prov} ({alt['model']})")
                alt_instance = get_provider_instance(alt_prov)
                if not alt_instance:
                    continue
                
                response = await alt_instance.generate(prompt=prompt, model=alt["model"], image_bytes=image_bytes, capability="fallback_recovery")
                if response.ok:
                    ...
                    self._log_immutable_transaction(
                        intent=decision.reason, request_id=request_id,
                        candidates=decision.metadata.get("candidates_log", []),
                        selected=decision.model, state="RECOVERY_SUCCESS",
                        execution_details={"original_provider": original_provider, "fallback_provider": alt_prov, "fallback_model": alt["model"]}
                    )
                    return response
                else:
        ...
        self._log_immutable_transaction(
            intent=decision.reason, request_id=request_id,
            candidates=decision.metadata.get("candidates_log", []),
            selected=decision.model, state=final_state,
