...
class DiscordPermissionGuard:
    ...
    def evaluate_guild_access(self, guild_id: str | None, user_id: str, username: str = None, is_dm: bool = False) -> dict:
        ...
        if is_dm:
            ...
            if policy.get("dm_owner_only", True):
                ...
                if user_id not in allowed_users:
                    ...
                    self._audit_log("access_denied.jsonl", {"timestamp": timestamp, "user_id": user_id, "username": username, "reason": "DM_UNAUTHORIZED_USER"})
                    return {"granted": False, "reason": "DM_UNAUTHORIZED_USER"}

            print(f"[OK] ACCÈS DM ACCORDÉ pour {user_id}")
            self._audit_log("access_granted.jsonl", {"timestamp": timestamp, "user_id": user_id, "username": username, "context": "DM", "status": "GRANTED"})
        ...
        if guild_id not in allowed_guilds:
            ...
            self._audit_log("access_denied.jsonl", {"timestamp": timestamp, "guild_id": guild_id, "user_id": user_id, "reason": "GUILD_NOT_AUTHORIZED"})
        ...
        if not user_check["granted"]:
            self._audit_log("access_denied.jsonl", {"timestamp": timestamp, "guild_id": guild_id, "user_id": user_id, "username": username, "reason": "USER_NOT_AUTHORIZED_IN_GUILD"})
            return user_check

        self._audit_log("access_granted.jsonl", {"timestamp": timestamp, "guild_id": guild_id, "user_id": user_id, "username": username, "status": "GRANTED"})
        return user_check
