...
def get_persona():
    try:
        ...
        return "Tu es E-ZZIO, l'ami IA local d'Enrik sur PC. Tu es optimisé CPU/RAM only, sans pub, et tu réponds en français de façon concrète et utile."

def build_options(organ_key="presence", speed="normal"):
    options = dict(BASE_OPTIONS)
    options.update(ORGAN_OPTIONS.get(organ_key, ORGAN_OPTIONS["presence"]))
    if speed == "fast": options.update(FAST_OPTIONS)
    elif speed == "deep": options.update(DEEP_OPTIONS)
    options["num_gpu"] = 0
    return options

def format_memory(context, speed="normal"):
    if not context or speed == "fast": return ""
    lines = []
...
async def query_model_async(prompt, model="qwen2.5:7b", context=None, system_note=None, organ_key="presence", speed="normal", timeout_sec=60, tactical=None):
    ...
    try:
        ...
        if not text:
            ...
            text = f"*(Erreur technique : Le modèle {model} a retourné une réponse vide via l'organe {organ_key}.)*"
        ...
        elapsed_ms = int((time.perf_counter() - started) * 1000)
        return {"ok": False, "model": model, "elapsed_ms": elapsed_ms, "text": f"*(Timeout : Le modèle {model} n'a pas répondu après {timeout_sec}s.)*", "timeout_sec": timeout_sec, "speed": speed}
    except Exception as exc:
        elapsed_ms = int((time.perf_counter() - started) * 1000)
        return {"ok": False, "model": model, "elapsed_ms": elapsed_ms, "text": f"*(Erreur technique : Le modèle {model} a planté ou n'est pas installé -> {exc})*", "timeout_sec": timeout_sec, "speed": speed}