...
class SnapshotRestoreEngine:
    ...
    def create_bootstrap_snapshot(self) -> Dict[str, Any]:
        ...
        final_manifest = {
            **manifest,
            "global_snapshot_sha256": global_hash
        }
    ...
    def restore_snapshot(self, snapshot_id: str) -> Dict[str, Any]:
        ...
        for file_info in manifest_data.get("files_captured", []):
            ...
            if actual_hash != expected_hash:
                raise SnapshotRestoreError(
                    f"FAIL CLOSED : Corruption détectée sur '{rel_path}' ! "
                    f"Attendu: {expected_hash[:12]} | Obtenu: {actual_hash[:12]}"
                )
        ...
        result = self.gateway.execute_via_gateway(
            action="ATOMIC_SNAPSHOT_RESTORE",
            payload=payload,
            target_func=execute_atomic_restore
        )
...
def test_snapshot_restore():
    ...
    try:
        ...
        print(f"  [PASS] Restauration atomique réussie !")
