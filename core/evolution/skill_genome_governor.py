...
class SkillGenomeEngine:
    ...
    def evaluate_and_promote_skill(
        ...
        if risk_score > 5.0:
            raise SkillAcquisitionError(f"FAIL CLOSED : Rejet de la skill '{skill_id}'. Score de risque trop élevé ({risk_score}/10).")
        ...
        result = self.gateway.execute_via_gateway(action="SKILL_ACQUIRE_PROMOTE", payload=payload, target_func=commit_skill)
