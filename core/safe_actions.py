...
def run_action_logic(action: str, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    if action == "brain_status":
        ...
        return {"ok": False, "error": "Bidi_Scanner_v2.ps1 introuvable."}
...
# ============================================================
# v2.21.3 SAFE ACTION HYGIENE - append-only cancellations
# ============================================================
...
def augment_queue_items(items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for item in items:
        ...
        proposal_id = clone.get("id")
        run = runs_by_proposal.get(proposal_id)
        cancel = cancels_by_proposal.get(proposal_id)
        ...
        if run:
            ...
            clone["run_elapsed_ms"] = run.get("elapsed_ms")
            clone["run_created_at"] = run.get("created_at")
            clone["cancel_id"] = None
            clone["cancel_reason"] = None
        elif cancel:
            clone["effective_status"] = "cancelled"
            clone["executed"] = False
            ...
            clone["run_elapsed_ms"] = None
            clone["run_created_at"] = None
            clone["cancel_id"] = cancel.get("id")
            clone["cancel_reason"] = cancel.get("reason")
            clone["cancel_created_at"] = cancel.get("created_at")
        else:
            clone["effective_status"] = "pending_confirmation" if clone.get("requires_confirmation") else "pending_read"
            clone["executed"] = False
            clone["cancelled"] = False
...
def ledger(limit: int = 100) -> Dict[str, Any]:
    ...
    pending = [item for item in items if not item.get("executed") and not item.get("cancelled")]
    pending_confirmation = [item for item in pending if item.get("requires_confirmation")]
    pending_read = [item for item in pending if not item.get("requires_confirmation")]
...
def status() -> Dict[str, Any]:
    ...
    return {
        ...
        "queue_path": str(QUEUE_PATH),
        "runs_path": str(RUNS_PATH),
        "cancellations_path": str(CANCELS_PATH),
        "known_actions": len(ACTION_REGISTRY),
        "queued_count": data["count"],
        "executed_count": data["executed_count"],
...
def cancel_proposal(proposal_id: str, reason: str = "annulation manuelle") -> Dict[str, Any]:
    ...
    if already_executed(proposal_id):
        return {
            "ok": False,
            "error": "Impossible d'annuler : proposition déjà exécutée.",
            "proposal_id": proposal_id,
        }

    if cancel_index().get(proposal_id):
        return {
            "ok": True,
            "already_cancelled": True,
            "proposal_id": proposal_id,
            "message": "Cette proposition était déjà annulée.",
        }
...
def cancel_pending(reason: str = "nettoyage file pending") -> Dict[str, Any]:
    ...
    for item in pending:
        result = cancel_proposal(item.get("id"), reason=reason)
        if result.get("ok"):
...
def run_proposal(proposal_id: str, confirmation: str = "") -> Dict[str, Any]:
    ...
    if cancel_index().get(proposal_id):
        return {
            "ok": False,
            "error": "Action refusée : proposition annulée.",
            "proposal_id": proposal_id,
        }
