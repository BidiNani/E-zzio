...
def _load_capabilities():
    ...
    return {
        ...
        "qwen2.5:7b": {"roles": ["reasoning", "deep_analysis", "architecture"], "speed": "medium"},
        "qwen2.5:7b": {"roles": ["quick_response"], "speed": "ultra-fast"},
...
def best_fast_model(candidates=None):
    ...
    sorted_by_speed = sorted(
        [c for c in pool if c in MODELS_DB],
        key=lambda x: SPEED_HIERARCHY.get(MODELS_DB[x].get("speed", "fast"), 5)
    )
