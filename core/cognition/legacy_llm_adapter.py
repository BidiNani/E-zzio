...
class LegacyLLMAdapter:
    ...
    def intercept_ollama_chat(self, model: str, messages: List[Dict[str, str]], **kwargs) -> Dict[str, Any]:
        ...
        if self.gateway:
            ecol_response = self.gateway.ask(task=task_description, priority="normal", context_payload=context_payload)
    ...
    def intercept_api_call(self, endpoint: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "choices": [{"message": {"role": "assistant", "content": "Requête API externe bloquée et traitée par ECOL."}}],
