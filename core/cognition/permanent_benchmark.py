...
class PermanentBenchmarkEngine:
    ...
    def run_benchmark_tick(self) -> Dict[str, Any]:
        ...
        # 3. Inspection du Decision Ledger
        decision_ledger_path = self.root_dir / "runtime" / "cognition" / "budget" / "unified_decision_ledger.jsonl"
        total_decisions = 0
        if decision_ledger_path.exists():
            with open(decision_ledger_path, "r", encoding="utf-8") as f:
                total_decisions = sum(1 for line in f if line.strip())
        ...
        benchmark_data = {
            ...
            "memory": {
                "total_records": total_mem_records,
                "tiers_distribution": tiers_count
            },
            "cognition": {
                "total_recorded_decisions": total_decisions
            },
...
def test_benchmark():
    ...
    print(f" Charge CPU / RAM    : {res['hardware']['cpu_usage_percent']}% / {res['hardware']['ram_usage_percent']}% ({res['hardware']['ram_available_gb']} Go dispo)")
