...
class ResilienceOrchestrator:
    ...
    def handle_network_outage(error: Exception) -> str:
        ...
        ledger.append_event("NETWORK_OUTAGE_HANDLED", {
            "circuit_state": kernel_circuit_breaker.state,
            "error_snippet": sanitized_err[:100]
        })
    ...
    def handle_vault_failure(error: Exception) -> dict:
        ...
        ledger = ImmutableAuditLedger(AUDIT_DIR / "security_events.jsonl")
        ledger.append_event("VAULT_FAILURE_FALLBACK", {
            "error": sanitized_err,
            "action": "FALLBACK_TO_ENV_VARIABLES"
        })
        ...
        return {
            ...
            "token_resolved": fallback_token is not None
        }
