...
class WindowsMemoryInspector:
    ...
    def get_system_memory_pressure() -> dict:
        ...
        try:
            ...
            ctypes.windll.kernel32.GlobalMemoryStatusEx(ctypes.byref(stat)) if hasattr(
            ...
            return {"memory_load_pct": load_pct, "total_ram_gb": total_gb, "available_ram_gb": avail_gb, "os_pressure": load_pct > 85.0}
        except Exception:
            # Fallback neutre si exécution hors environnement Windows natif
            return {"memory_load_pct": 50.0, "total_ram_gb": 32.0, "available_ram_gb": 16.0, "os_pressure": False}
