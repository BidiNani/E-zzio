...
<#
.SYNOPSIS
    E-ZZIO — SEMANTIC TRUTH ENGINE v0.3.0
...
.DESCRIPTION
    ...
    Principes :
      ...
      4. Découverte dynamique de l'environnement forensic.
      5. Aucune supposition sur l'arborescence SelfBody.
      6. Validation SHA-256 avant analyse.
      7. Traitement streaming des records.
      8. Pas de chargement massif de semanticRecords en RAM.
      9. Aucun CERTIFIED/FROZEN avant validation post-commit.
     10. Toute erreur fatale produit un verdict FAIL-CLOSED.
#>
...
function Fail-Closed {
    ...
    if ($Exception) {
        Write-Host "TYPE  : $($Exception.GetType().FullName)" -ForegroundColor DarkRed
    ...
    if ($Exception) {
        Write-Log -Level FAIL -Message "$($Exception.GetType().FullName): $($Exception.Message)"
    }
...
function Get-CanonicalRunId {
    param(
        [Parameter(Mandatory)][string]$PhysicalDir,
        [Parameter(Mandatory)][string]$ManifestRunId
    )

    $leaf = Split-Path -Leaf $PhysicalDir

    if (-not $leaf.StartsWith('run_', [System.StringComparison]::Ordinal)) {
        throw "Le dossier PointZero '$leaf' ne respecte pas le préfixe canonique 'run_'."
    }
...
function Test-PathInside {
    ...
    return (
        ...
        $childFull.StartsWith($childPrefix, [System.StringComparison]::OrdinalIgnoreCase)
    )
}
...
function Find-TopologyArtifact {
    param(
        [Parameter(Mandatory)][string]$ForensicRoot,
    ...
    $candidatePaths.Add(
        (Join-Path $ForensicRoot "SelfBody\topology\run_$RunId\$expectedName")
    )

    $candidatePaths.Add(
        (Join-Path $ForensicRoot "SelfBody\semantic\topology\run_$RunId\$expectedName")
    )

    $candidatePaths.Add(
        (Join-Path $ForensicRoot "topology\run_$RunId\$expectedName")
    )

    $outputParent = Split-Path $OutputDir -Parent

    $candidatePaths.Add(
        (Join-Path $outputParent "..\topology\run_$RunId\$expectedName")
    )
    ...
    #
    # IMPORTANT :
    #   On ne fait PAS de Get-ChildItem massif sur tout G:\.
    #   Recherche limitée à la racine forensic.
    # ------------------------------------------------------------------------
    ...
    if ([System.IO.Directory]::Exists($ForensicRoot)) {
        ...
        $matches = Get-ChildItem `
            ...
            Where-Object {
                $_.FullName -match "\\run_$([regex]::Escape($RunId))\\$([regex]::Escape($expectedName))$"
            }
        ...
        if ($matches.Count -gt 1) {
            throw "Ambiguïté topologique : plusieurs EZZIO_TOPOLOGY_STATE.json correspondent au RunId $RunId."
        }
    }
...
$forensicRoot = Resolve-CanonicalPath (
    Split-Path (Split-Path $resolvedPointZero -Parent) -Parent
)

$OutputStatePath    = Join-Path $resolvedOutput 'EZZIO_SEMANTIC_STATE.json'
$OutputManifestPath = Join-Path $resolvedOutput 'semantic_manifest.json'
$OutputFrozenPath   = Join-Path $resolvedOutput 'FROZEN'
$OutputPayloadPath  = Join-Path $resolvedOutput 'semantic_payload.jsonl'
$OutputLogPath      = Join-Path $resolvedOutput 'semantic_engine.log'

# ============================================================================
# HEADER
# ============================================================================

Write-Section "E-ZZIO — SEMANTIC TRUTH ENGINE v$EngineVersion"
...
try {
    ...
    Write-Log -Level INFO -Message "Semantic Truth Engine $EngineVersion démarré."
    Write-Log -Level INFO -Message "PointZero=$resolvedPointZero"
    Write-Log -Level INFO -Message "Output=$resolvedOutput"
    ...
    if (Test-PathInside -Child $resolvedOutput -Parent $resolvedPointZero) {
        throw "OutputDir situé à l'intérieur du PointZero."
    }
    ...
    Write-Section "SG-01 — IDENTITÉ CANONIQUE"

    $ManifestPath = Join-Path $resolvedPointZero 'run_manifest.json'
    $RecordsPath  = Join-Path $resolvedPointZero 'records.jsonl'
    ...
    Write-Pass "Identité canonique : $canonicalRunId"
    ...
    Write-Host "Recherche de EZZIO_TOPOLOGY_STATE.json..." -ForegroundColor Yellow
...
"@
    ...
    $topologyStatePath = Resolve-CanonicalPath $topologyStatePath

    Write-Pass "Topologie découverte automatiquement"
    Write-Host "Topology : $topologyStatePath"

    $topologyState = Read-JsonStrict -Path $topologyStatePath

    $topologyRunId = [string]$topologyState.lineage.parent_point_zero_run
    $topologyRecordsSha = [string]$topologyState.lineage.parent_records_sha256

    if ($topologyRunId -cne $canonicalRunId) {
        throw "Topologie RunId incohérent : $topologyRunId != $canonicalRunId"
    }

    if ($topologyRecordsSha -cne $recordsSha) {
        throw "Topologie/records SHA-256 incohérents."
    }

    $topologyPayloadSha = [string]$topologyState.proof.topology_payload_sha256

    if ([string]::IsNullOrWhiteSpace($topologyPayloadSha)) {
        throw "topology_payload_sha256 absent."
    }

    $topologyArtifactSha = Get-FileSha256 $topologyStatePath

    Write-Pass "SG-02_TOPOLOGY_LINEAGE_VALIDATED"

    # ========================================================================
    # SG-03 — FROZEN EXISTANT
    # ========================================================================
...
"@
    ...
    if ($Force -and [System.IO.Directory]::Exists($resolvedOutput)) {

        # IMPORTANT :
        # On ne supprime PAS le répertoire.
        # On remplace uniquement les artefacts appartenant à ce moteur.
        ...
        Write-Info "Anciens artefacts sémantiques supprimés sous -Force."
    }
    ...
    try {
        ...
        while (-not $Reader.EndOfStream) {
            ...
            try {
                ...
                throw "records.jsonl invalide à l'enregistrement $processedCount : $($_.Exception.Message)"
            }
            ...
            if ([string]::IsNullOrWhiteSpace($relPath)) {
                throw "RelativePath vide à l'enregistrement $processedCount."
            }

            if ([string]::IsNullOrWhiteSpace($recordHash)) {
                throw "Hash vide pour '$relPath'."
            }
            ...
            $physicalPath = [System.IO.Path]::GetFullPath(
                (Join-Path $resolvedPointZero $relPath)
            )
            ...
            if (-not (Test-PathInside `
                ...
                throw "PATH TRAVERSAL détecté : $relPath"
            }
...
dans le Point Zero. FAIL-CLOSED.
"@
            }
            ...
            $actualSha256 = Get-FileSha256 $physicalPath

            if ($actualSha256 -cne $recordHash.ToLowerInvariant()) {
...
"@
            ...
            try {
                ...
                switch ($ext) {
                    ...
                    '.py' {
                        ...
                        # IMPORTANT :
                        # v0.3 ne prétend PAS faire un AST Python complet.
                        # Il établit uniquement l'identité module.
                        #
                        # Pas de faux PARSED.
    ...
    if (-not $allGatesPass) {
        throw 'Une ou plusieurs Quality Gates ont échoué.'
    }
    ...
    $payloadSha256 = Get-FileSha256 $OutputPayloadPath

    Write-Host "Semantic Payload SHA256 : $payloadSha256"
    ...
    $stateObj = [ordered]@{
        ...
        lineage = [ordered]@{
            parent_point_zero_run       = $canonicalRunId
            parent_manifest_sha256      = $manifestSha
            parent_records_sha256       = $recordsSha
            parent_topology_payload_sha = $topologyPayloadSha
            parent_topology_artifact_sha256 = $topologyArtifactSha
        }

        semantic_payload = [ordered]@{
            format = 'JSONL'
            path   = $OutputPayloadPath
            sha256 = $payloadSha256
        }

        statistics = [ordered]@{
            records_read       = $processedCount
            files_analyzed     = $parsedCount
    ...
    $stateSha256 = Get-FileSha256 $OutputStatePath

    Write-Pass "EZZIO_SEMANTIC_STATE.json écrit"
    ...
    $manifestObj = [ordered]@{
        ...
        parent_topology_payload_sha256 = $topologyPayloadSha

        parent_topology_artifact_sha256 = $topologyArtifactSha

        semantic_payload_sha256 = $payloadSha256

        semantic_state_sha256 = $stateSha256
        ...
        status = 'VALIDATED_SEMANTIC_DERIVATION'
    ...
    $semanticManifestSha = Get-FileSha256 $OutputManifestPath

    Write-Pass "semantic_manifest.json écrit"
    ...
    foreach ($required in @(
        ...
        if (-not [System.IO.File]::Exists($required)) {
            throw "Artefact final absent : $required"
        }

        Write-Pass ([System.IO.Path]::GetFileName($required))
    }

    $recheckPayloadSha = Get-FileSha256 $OutputPayloadPath
    $recheckStateSha   = Get-FileSha256 $OutputStatePath
    $recheckManifestSha = Get-FileSha256 $OutputManifestPath

    if ($recheckPayloadSha -cne $payloadSha256) {
        throw 'Payload SHA-256 instable après commit.'
    }

    if ($recheckStateSha -cne $stateSha256) {
        throw 'State SHA-256 instable après commit.'
    }

    if ($recheckManifestSha -cne $semanticManifestSha) {
        throw 'Manifest SHA-256 instable après commit.'
    }
...
"@
    ...
    $frozenSha256 = Get-FileSha256 $OutputFrozenPath
    ...
    Write-Section "E-ZZIO — NIVEAU 3 : CERTIFIED & FROZEN"
    ...
    Write-Host ' ENVIRONMENT / TOPOLOGY / SEMANTIC TRUTH : VALIDATED' -ForegroundColor Green
    Write-Host ' CERTIFIED : TRUE' -ForegroundColor Green
    Write-Host ' FROZEN    : TRUE' -ForegroundColor Green
    Write-Host '==============================================================================' -ForegroundColor Green

    Write-Log -Level PASS -Message 'Semantic Truth Derivation certifiée et frozen.'
    ...
    Write-Host ''
    Write-Host 'Le moteur s''est arrêté proprement.' -ForegroundColor Yellow
    Write-Host 'Le Point Zero n''a pas été modifié.' -ForegroundColor Yellow
    Write-Host ''
