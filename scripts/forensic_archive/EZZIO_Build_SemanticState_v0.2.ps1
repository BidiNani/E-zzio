...
<#
.SYNOPSIS
    E-ZZIO — SEMANTIC TRUTH ENGINE v0.2 (NIVEAU 3)
.DESCRIPTION
    Dérivation sémantique stricte du Point Zéro et de l'État Topologique.
    - Zéro supposition sémantique (Parsing AST natif).
    - Résolution stricte et normalisée de l'identité du RunId.
    - Ancrage absolu des symboles au RecordHash physique.
    - FAIL-CLOSED intégral.
#>
...
function Get-FileSha256 {
    ...
    if (-not [System.IO.File]::Exists($Path)) { Fail-Closed "Fichier absent : $Path" }
...
function Assert-CanonicalPointZeroIdentity {
    ...
    if (-not $leaf.StartsWith('run_')) {
        Fail-Closed "Le dossier cible '$leaf' ne possède pas le préfixe canonique 'run_'."
    }

    $normalizedId = $leaf.Substring(4)

    if ($normalizedId -notmatch '^\d{8}_\d{6}_\d{3}$') {
        Fail-Closed "Identité physique malformée : '$normalizedId'."
    }

    if ($normalizedId -ne $LogicalManifestId) {
        Fail-Closed "Rupture d'identité. Manifest ($LogicalManifestId) != Physique normalisé ($normalizedId)."
    }
...
# ============================================================================
# 2. CANONICAL JSON E-ZZIO (Pour Déterminisme du Payload)
# ============================================================================
...
# Application du correctif d'identité stricte
...
# Résolution dynamique et étanche de l'artefact Topologique
$topologyStatePath = Join-Path (Split-Path $PointZeroDir -Parent) "SelfBody\topology\run_$canonicalRunId\EZZIO_TOPOLOGY_STATE.json"
$topologyStatePath = [System.IO.Path]::GetFullPath($topologyStatePath)

if (-not [System.IO.File]::Exists($topologyStatePath)) {
    # Fallback si l'arbre SelfBody a une racine différente
    $topologyStatePath = Join-Path (Split-Path $OutputDir -Parent) "topology\run_$canonicalRunId\EZZIO_TOPOLOGY_STATE.json"
}
...
try {
    while (-not $reader.EndOfStream) {
        ...
        $physicalPath = Join-Path $PointZeroDir $relPath

        # Filtre de traitement linguistique
        if ($ext -notin @('.ps1', '.py', '.json', '.gd', '.lua')) { continue }

        # Vérification physique stricte avant parsing (Contrat Niveau 3)
        $actualSha256 = Get-FileSha256 -Path $physicalPath
        if ($actualSha256 -ne $recordHash) {
            Fail-Closed "Mutation physique détectée sur $relPath (Attendu: $recordHash, Actuel: $actualSha256)"
        }
        ...
        try {
            switch ($ext) {
                '.json' {
                    # Parsing JSON strict (Validation structurelle)
                ...
                '.py' {
                    # Placeholder Python (AST à injecter)
                    $parserStatus = "PARSED"
        ...
        # Ancrage obligatoire de la provenance
...
$gates['SG-02_PHYSICAL_PARITY']      = ($failedCount -eq 0) # Tolérance pour UNSUPPORTED, pas pour ERROR
$gates['SG-03_PROVENANCE_LINKED']    = ($semanticRecords.Count -gt 0)

# Tri ordinal pour déterminisme
