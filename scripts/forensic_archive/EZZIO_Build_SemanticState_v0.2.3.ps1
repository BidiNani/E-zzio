...
<#
.SYNOPSIS
    E-ZZIO — SEMANTIC TRUTH ENGINE v0.2.3 (NIVEAU 3)
.DESCRIPTION
    Dérivation sémantique stricte du Point Zéro et de l'État Topologique.
    - Zéro supposition sémantique (Parsing AST natif).
    - Pont forensique : Confrontation du Point Zéro (figé) avec le SourceRoot (vivant).
...
if (-not [System.IO.Directory]::Exists($SourceRoot)) {
    Fail-Closed "SourceRoot introuvable : $SourceRoot"
}
