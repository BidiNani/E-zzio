...
<#
.SYNOPSIS
    E-ZZIO — SEMANTIC TRUTH ENGINE v0.3.2

.DESCRIPTION
    Moteur de dérivation sémantique forensic de niveau 3.

    v0.3.2 corrige la résolution du SourceRoot.
    ...
    Principes :
      ...
      4. SourceRoot explicitement dérivé du manifest.
      5. Aucune supposition silencieuse sur le corpus physique.
      6. Découverte dynamique contrôlée de la topologie.
      7. Validation SHA-256 avant analyse.
      8. Traitement streaming des records.
      9. Faible pression mémoire.
     10. Aucun CERTIFIED/FROZEN avant validation post-commit.
     11. Toute erreur fatale produit un verdict FAIL-CLOSED.
#>
...
# ============================================================================
# SOURCE ROOT — RÉSOLUTION EXPLICITE
# ============================================================================

function Resolve-ManifestSourceRoot {
    param(
        [Parameter(Mandatory)]
        [object]$Manifest
    )
    ...
    #     $Manifest.SourceRoot
    #
    # car StrictMode provoque une PropertyNotFoundException si la propriété
    # n'existe pas.
    #
    # Les formes ci-dessous sont les seules formes explicitement supportées.
    # Aucune reconstruction arbitraire du SourceRoot n'est effectuée.
    # ------------------------------------------------------------------------
    ...
    $candidates.Add(
        [ordered]@{
            Path = 'SourceRoot'
            Value = Get-JsonPropertyValue `
                -Object $Manifest `
                -PropertyName 'SourceRoot'
        }
    )
    ...
    $candidates.Add(
        [ordered]@{
            ...
            Value = Get-JsonPropertyValue `
                -Object $Manifest `
                -PropertyName 'source_root'
        }
    )

    $sourceObject = Get-JsonPropertyValue `
        -Object $Manifest `
        -PropertyName 'source'
    ...
    $environmentObject = Get-JsonPropertyValue `
        -Object $Manifest `
    ...
    $provenanceObject = Get-JsonPropertyValue `
        -Object $Manifest `
        -PropertyName 'provenance'

    if ($null -ne $provenanceObject) {

        $candidates.Add(
            [ordered]@{
                Path = 'provenance.SourceRoot'
                Value = Get-JsonPropertyValue `
    ...
    if ($valid.Count -eq 0) {
        ...
        Write-Host ''
        Write-Host 'SOURCE ROOT ABSENT DU MANIFEST' -ForegroundColor Red
        Write-Host ''

        Write-Host 'Propriétés top-level réellement présentes :' `
        ...
        throw @"
SourceRoot introuvable dans run_manifest.json.

Aucune forme explicitement supportée n'a été trouvée.
...
"@
    ...
    if ($valid.Count -gt 1) {
        ...
        if ($distinctValues.Count -gt 1) {
            ...
            Write-Host ''
            Write-Host 'AMBIGUÏTÉ SOURCEROOT' -ForegroundColor Red
            Write-Host ''
            ...
            throw `
                'Plusieurs SourceRoot contradictoires sont présents dans le manifest.'
        }
    }

    $selected = $valid[0]

    $sourceRoot = Resolve-CanonicalPath $selected.Value

    if (-not [System.IO.Directory]::Exists($sourceRoot)) {
        throw @"
SourceRoot déclaré mais inaccessible.

ManifestPath : $($selected.Path)
SourceRoot   : $sourceRoot

Le moteur refuse toute substitution automatique.
"@
    }

    Write-Pass (
        "SourceRoot résolu via manifest : {0}" -f $selected.Path
    )

    Write-Host "SourceRoot live : $sourceRoot" `
        -ForegroundColor DarkGray
...
try {
    ...
    $sourceRoot = Resolve-ManifestSourceRoot `
        -Manifest $manifest

    # ------------------------------------------------------------------------
    # Frontière SourceRoot / PointZero
    # ------------------------------------------------------------------------

    if (Test-PathInside `
        -Child $resolvedPointZero `
        -Parent $sourceRoot) {

        Write-Warn `
            'Le PointZero se trouve dans SourceRoot ; cette relation est observée mais ne modifie pas le PointZero.'
    }

    $manifestSha = Get-FileSha256 `
        $ManifestPath

    $recordsSha = Get-FileSha256 `
        $RecordsPath

    Write-Host "Manifest SHA256 : $manifestSha"
    Write-Host "Records SHA256  : $recordsSha"

    Write-Pass "Manifest SHA-256 validé"
    Write-Pass "Records SHA-256 validé"
...
"@
    ...
    $lineage = Get-JsonPropertyValue `
        -Object $topologyState `
        -PropertyName 'lineage'

    if ($null -eq $lineage) {
        throw 'Section lineage absente de EZZIO_TOPOLOGY_STATE.json.'
    }

    $topologyRunId = [string](
        Get-JsonPropertyValue `
            -Object $lineage `
            -PropertyName 'parent_point_zero_run'
    )

    $topologyRecordsSha = [string](
        Get-JsonPropertyValue `
            -Object $lineage `
            -PropertyName 'parent_records_sha256'
    )
    ...
    $proof = Get-JsonPropertyValue `
        -Object $topologyState `
        -PropertyName 'proof'

    if ($null -eq $proof) {
        throw 'Section proof absente de EZZIO_TOPOLOGY_STATE.json.'
    }

    $topologyPayloadSha = [string](
        Get-JsonPropertyValue `
            -Object $proof `
            -PropertyName 'topology_payload_sha256'
    )
    ...
    $topologyArtifactSha = Get-FileSha256 `
        $topologyStatePath
...
"@
    ...
    try {
        ...
        while (-not $Reader.EndOfStream) {
            ...
            # ----------------------------------------------------------------
            # Protection path traversal — SOURCE ROOT
            # ----------------------------------------------------------------
            ...
            try {
                ...
                switch ($ext) {
                    ...
                    '.py' {
                        ...
                        # ----------------------------------------------------
                        # v0.3.x ne prétend PAS produire un AST Python complet.
                        # ----------------------------------------------------
...
"@
    ...
    Write-Log `
        -Level PASS `
        -Message 'Semantic Truth Derivation certifiée et frozen.'
