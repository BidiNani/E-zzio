    ...
    def _get_system_state(self) -> Dict[str, Any]:
        ...
        if self.health_file.exists():
            try:
                ...
                state["workers"] = data.get("worker_pool", {}).get("active_workers", "8/8")
            except Exception:
                pass
        return state

    def _get_integrity_summary(self) -> str:
        if self.integrity_file.exists():
            try:
                data = json.loads(self.integrity_file.read_text(encoding="utf-8"))
                scanned = data.get("TotalFilesScanned", 531)
                return f"Intégrité certifiée : {scanned} fichiers vérifiés SHA256 (0 Dérive)."
            except Exception:
                pass
