...
class EzzioKernel:
    ...
    def run_diagnostics(self) -> dict:
        ...
        return {"persona": persona_ok, "memory": memory_ok, "state": state_ok, "ollama": ollama_ok}
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("  • Persona compilée : " + ("✅ OK" if diag["persona"] else "❌ ABSENTE"))
    print("  • Mémoire de session : " + ("✅ OK" if diag["memory"] else "⚠ Initialisée"))
    print("  • État système (State) : " + ("✅ OK" if diag["state"] else "⚠ Initialisé"))
    print("  • Moteur Ollama local : " + ("✅ OK (Connecté)" if diag["ollama"] else "❌ INJOIGNABLE"))
    print("  • Agent Control Loop : ✅ Active (Multi-tours)")
