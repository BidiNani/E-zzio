"""Runtime Context Builder — Consommateur strict de CanonicalIdentity (Fail-Closed)."""
...
def get_base_system_prompt() -> str:
    """Retourne l'identité canonique sans aucun fallback textuel (Fail-Closed strict)."""
    global _CANONICAL_IDENTITY
    if _CANONICAL_IDENTITY is None:
        try:
            _CANONICAL_IDENTITY = CanonicalIdentity(root_dir=PROJECT_ROOT)
        except Exception as exc:
            raise RuntimeError(f"[IDENTITY FAIL-CLOSED] ContextBuilder initialisation impossible : {exc}") from exc
