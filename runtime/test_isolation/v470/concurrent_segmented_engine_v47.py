...
class L1MemoryIndex:
    """Index sémantique L1 à ultra-basse latence en mémoire RAM."""
    def __init__(self):
    ...
    def query_by_agent(self, agent_id: str) -> list:
        with self._lock:
            return list(self.by_agent.get(agent_id, []))

    def query_by_type(self, semantic_type: str) -> list:
        with self._lock:
            return list(self.by_type.get(semantic_type, []))
...
class ConcurrentSegmentedEngineV47:
    ...
    def _reconstruct_manifest_from_disk(self) -> dict:
        ...
        if self.segments_dir.exists():
            for p in sorted(self.segments_dir.glob("segment_*.jsonl")):
                ...
                is_sealed, _ = self._inspect_segment_footer(p)
    ...
    def write_event(self, event_data: dict):
        ...
        # 1. Indexation instantanée L1 en RAM
        self.l1_index.index_event(event_data)
        
        # 2. Enfilement pour persistance L2 async
