...
class MockMemoryEngine:
    def __init__(self):
        ...
        self.state = {"oom_index": 0.2, "d_rss_dt_mbs": 0.0, "rss_mb": 2048.0, "critical_oom_risk": False}
...
def test_v60_semantic_backpressure_certification():
    ...
    try:
        ...
        controller.update_backpressure_state()
        print(
            f"[NOMINAL] N_ctx LLM: {controller.get_llm_context_budget()} | Agents Arrière-plan Autorisés: {controller.is_agent_execution_allowed(True)}",
            flush=True,
        )

        assert controller.get_llm_context_budget() == 16384, "FAIL: Fenêtre de contexte nominale attendue à 16384 !"
        assert controller.is_agent_execution_allowed(True), "FAIL: Les agents d'arrière-plan doivent être autorisés !"
        ...
        print(
            f"[THROTTLED] Tier: {state2['regulation_tier']} | N_ctx LLM: {controller.get_llm_context_budget()} | Ingestion Delay: {controller.acquire_ingestion_slot()}s",
            flush=True,
        )
        ...
        assert not controller.is_agent_execution_allowed(True), "FAIL: Les agents d'arrière-plan auraient dû être suspendus !"
