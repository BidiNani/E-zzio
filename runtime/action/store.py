...
class ActionStore:
    ...
    def load_contracts(self) -> List[Dict[str, Any]]:
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute(
                "SELECT id, version, name, description, permission, handler_ref, cost, timeout, risk_level, schema FROM action_contracts"
            )
            return [
                {
                    "id": row[0],
    ...
    def get_transition_history(self, exec_id: str) -> List[Tuple[str, str]]:
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute("SELECT from_state, to_state FROM state_transitions WHERE exec_id = ? ORDER BY rowid ASC", (exec_id,))
    ...
    def get_evidence_history(self, limit: int = 50) -> List[Dict[str, Any]]:
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute(
                "SELECT evidence_id, exec_id, root_trace_id, action_name, state, risk_level, duration_ms, timestamp FROM evidence_ledger limit ?",
                (limit,),
            )
            return [
                {
