...
class AgentLoop:
    ...
    def run(self, objective):
        ...
        if not self.constitution.validate_action(objective):
            ...
            return {
                "state": "HALTED",
                "reason": "Constitution violation: Immutable path target detected",
        ...
        try:
            episode = self.episode_extractor.extract(
                task_id=task.task_id, objective=objective, status=status_str, ledger_events=self.ledger.events
            )
