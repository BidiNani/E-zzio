...
class IncidentStore:
    """Moteur de stockage SQLite Haute Performance avec verrous RLock et Export JSON."""
    ...
    def _init_schema(self):
        with self._conn:
            ...
            if version < 2:
                ...
                self._conn.execute("CREATE INDEX IF NOT EXISTS idx_inc_category ON incident_bundles(category);")
                self._conn.execute("CREATE INDEX IF NOT EXISTS idx_inc_exec ON incident_bundles(execution_id);")
                
                self._conn.execute("INSERT OR REPLACE INTO incident_schema (version) VALUES (2)")
        ...
        with self._db_lock:
            with self._conn:
                self._conn.execute('''
                    ...
                    (incident_id, timestamp, severity, severity_score, category, execution_id, action_name, context_valid, payload_hash, bundle_hash, state_trace, telemetry_snapshot, root_candidates, metadata)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (
                    bundle.incident_id,
                    bundle.timestamp,
    ...
    def export_bundle_json(self, bundle: IncidentBundle) -> str:
        """Sauvegarde indépendante au format JSON pour survie aux pannes SQLite."""
