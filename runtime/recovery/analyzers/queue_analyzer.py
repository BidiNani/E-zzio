...
class QueueAnalyzer(BaseAnalyzer):
    def analyze(self, context: Dict[str, Any]) -> List[Finding]:
        ...
        if backlog > 5000:
            findings.append(Finding(type="QUEUE_CONGESTION", confidence=0.88, severity="WARNING", evidence={"queue_backlog": backlog}))
        if dropped > 0:
            findings.append(Finding(type="TELEMETRY_DROPS", confidence=0.99, severity="HIGH", evidence={"dropped_count": dropped}))
        return findings
