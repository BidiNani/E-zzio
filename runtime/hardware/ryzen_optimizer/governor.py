        ...
        if new_profile != self.current_profile:
            ...
            if (now - self.last_transition) >= self.hold_time:
                self.decision_ledger.append(
                    {"timestamp": time.time(), "from": self.current_profile, "to": new_profile, "reason": "HYSTERESIS_HOLD_EXPIRED"}
                )
...
class RyzenResourceGovernor:
    ...
    def evaluate_and_scale(self, spi: float, temperature: float = 50.0) -> dict:
        with self._lock:
            ...
            if adjusted_spi < 0.30:
                target = "IDLE"
            elif adjusted_spi < 0.70:
                target = "NORMAL"
            elif adjusted_spi < 0.90:
                target = "HEAVY"
            else:
                target = "EMERGENCY"
