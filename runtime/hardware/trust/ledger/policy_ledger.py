...
class PolicyAuditLedger:
    ...
    def log_decision(self, trust_context: dict, request: dict, decision_result: dict, policy_version: str):
        record = {
            "timestamp": time.time(),
            "trust_before": {"score": trust_context.get("trust_score"), "state": trust_context.get("state")},
            "workload": {"workload_id": request.get("workload_id"), "profile": request.get("profile")},
