# ==============================================================================
# E-ZZIO Runtime — Industrial Production Kernel V6.6.0 (Zero-Trust Governor)
# ==============================================================================
VERSION = "6.6.0"
...
class DecisionPolicy:
    ...
    reason: str
    is_safe_mode: bool = False  # Ajout du champ manquant

class DeepLearningEngine:
    """Heuristique d'apprentissage profond évaluant la sévérité et la durée de récupération."""
...
class PredictiveDecisionEngine:
    ...
    def evaluate(self, metrics: dict, context: dict) -> DecisionPolicy:
        ...
        if prev in ["THROTTLING_ACTIVE", "PRE_CRITICAL"]:
            if (now - context["last_transition"]) < p["hold"]: nxt, reason = prev, "HOLD_TIME_ACTIVE"
            elif oom <= p["exit"]: nxt, reason = "RECOVERY_CONFIRMED", "SAFE_WATERMARK_REACHED"
        
        if nxt == prev and nxt not in ["THROTTLING_ACTIVE", "PRE_CRITICAL"]:
            if ttc <= 12.0 or oom >= p["entry"]: nxt, reason = "THROTTLING_ACTIVE", f"CRITICAL_TRIGGER (TTC={ttc:.1f}s)"
            elif oom >= p["pre_entry"] or (0.0 < ttc <= 25.0): nxt, reason = "PRE_CRITICAL", "PREEMPTIVE_WARNING"
            elif oom >= (p["entry"] * 0.60): nxt, reason = "PRESSURE_DETECTED", "ELEVATED_PRESSURE"
            else: nxt, reason = "NORMAL_OPERATION", "NOMINAL"

        confidence = self.calculate_confidence(oom, d_rss, ttc, context["incident_count"])
        
        if nxt == "THROTTLING_ACTIVE": batch, delay, tier, poll = 250, 0.05, "CRITICAL", 0.025
        elif nxt == "PRE_CRITICAL": batch, delay, tier, poll = 1500, 0.02, "MODERATE", 0.100
        elif nxt == "PRESSURE_DETECTED": batch, delay, tier, poll = 2200, 0.015, "MODERATE", 0.250
        elif nxt in ["RECOVERY_PENDING", "RECOVERY_CONFIRMED"]: batch, delay, tier, poll = 2000, 0.012, "NOMINAL", 0.200
        else: batch, delay, tier, poll = 3000, 0.01, "NOMINAL", 0.500
...
class ProductionKernelV66:
    HMAC_SECRET = b"E-ZZIO-V66-ZERO-TRUST-KEY"
    ...
    def __init__(self, engine_ref, memory_engine_ref, state_dir: str = None, safe_cooldown: float = 60.0):
        ...
        self.state_file = self.state_dir / "production_v66_state.json"
        ...
        self.context = {"workload": "INGESTION_BURST", "current_state": "NORMAL_OPERATION", "last_transition": time.time(), "incident_count": 0, "current_polling": 2.0}
    ...
    def save_persisted_state_atomic(self):
        ...
        payload = {"version": VERSION, "current_state": self.context["current_state"], "workload": self.context["workload"], "timestamp": time.time()}
        payload["signature"] = self._generate_hmac(payload)
        try:
            with open(tmp, "w", encoding="utf-8") as f: json.dump(payload, f); f.flush(); os.fsync(f.fileno())
    ...
    def _watchdog_loop(self):
        while not self._stop_event.is_set():
            ...
            if not self.governor_thread.is_alive():
    ...
    def _execute_gradual_recovery(self):
        with self._lock:
            ...
            if self.safe_mode_step < len(recovery_steps):
                ...
                self.executor.apply(DecisionPolicy("SAFE_MODE_RECOVERING", "MODERATE", 1.0, 99.0, 1.0, batch, 0.02, "GRADUAL_RECOVERY", True))
    ...
    def _governor_loop(self):
        while not self._stop_event.is_set():
            ...
            if getattr(self, '_fatal_simulate', False):
                self._fatal_simulate = False
                raise BaseException("FATAL KERNEL EXCEPTION")
    ...
    def simulate_fatal_crash(self): self._fatal_simulate = True
    def simulate_thread_stall(self): self._stall_simulate = True
    def restore_thread_stall(self): self._stall_simulate = False

    def stop(self):
        self._stop_event.set()
        if self.governor_thread.is_alive(): self.governor_thread.join(timeout=1.0)
        if self.watchdog_thread.is_alive(): self.watchdog_thread.join(timeout=1.0)
