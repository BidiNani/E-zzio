...
def test_pure_cpu_governor():
    ...
    try:
        ...
        print(f"[IPC STATE READ] Profile: {state.get('active_profile')} | Version: {state.get('version')} | Threads: {state.get('cpu_topology', {}).get('assigned_affinity_count')}", flush=True)
