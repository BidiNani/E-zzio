...
print("=" * 60)
print(" E-ZZIO V7 — DIAGNOSTIC FORENSIQUE R3 (FAIL-CLOSED ABSOLU)")
print("=" * 60)
...
async def run_forensic_audit():
    ...
    payload = {
        "model": TARGET_MODEL,
        "prompt": "Réponds par un seul mot : bonjour.",
        "stream": True,
    ...
    print("\n--- [4/10] Test du parser JSON et inspection des tokens ---")
    extracted_tokens = []
                ...
                async for line in resp.content:
                    if line:
                        ...
                        if chunk_str:
                            ...
                            # Inspection forensic des clés reçues si le token est vide
                            token = data.get("response", "")
                            if not token and "message" in data: # Support endpoint chat implicite si besoin
                            ...
                            if token:
                                ...
                                # Affiche les clés reçues pour diagnostic si le chunk ne contient pas de réponse textuelle
                                if data.get("done", False):
                                    print(f"      [DEBUG] Chunk de fin détecté : {list(data.keys())}")
                                else:
                                    print(f"      [DEBUG] Chunk sans clé 'response' : {data}")
        
        ttft_val = ttft if ttft is not None else 0.0
        full_res = "".join(extracted_tokens).strip()
        print(f"   ℹ️ Tokens extraits : \"{full_res}\" (Total : {len(extracted_tokens)}, TTFT: {round(ttft_val, 2)} ms)")
    ...
    try:
        ...
        results["[5] Worker LLM opérationnel"] = False
    ...
    print(f"   ℹ️ Tokens récupérés depuis la queue : {len(queue_tokens)} tokens.")
    ...
    print(f"   ℹ️ Segments de phrases générés : {chunks_found}")
    ...
    try:
        ...
        results["[8] Kokoro TTS initialisé"] = False
    ...
    try:
        ...
        test_sentence = chunks_found[0] if chunks_found else "Bonjour Bidi."
        ...
        results["[9] Kokoro produit du PCM"] = False
    ...
    print("\n" + "=" * 60)
    print(" BILAN DU DIAGNOSTIC FORENSIQUE R3")
    print("=" * 60)
    all_passed = True
    ...
    if not all_passed:
        print("🔴 RÉSULTAT : ÉCHEC DE LA CERTIFICATION (FAIL-CLOSED APPLIQUÉ)")
        sys.exit(1)
    else:
        print("✨ RÉSULTAT : CERTIFICATION V6.3-R3 INTÉGRALE VALIDÉE (10/10)")
        print("=" * 60)
