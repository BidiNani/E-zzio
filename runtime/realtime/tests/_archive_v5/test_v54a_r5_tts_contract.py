...
class FakeTTS_ValidWAV:
    def __init__(self):
        ...
        self.tmp_file.close() # Fermer pour laisser wave l'ouvrir proprement
        
        # Création d'un vrai fichier WAV RIFF PCM 16kHz Mono 16-bit (~100ms de silence)
    ...
    async def synthesize(self, text):
        # Simulation d'un temps de traitement TTS proportionnel (ex: 150ms)
...
async def test_v54a_r5():
    print("=" * 60)
    print(" E-ZZIO V7 — V5.4a-R5 TTS BOUNDARY CERTIFICATION")
    print("=" * 60)

    fake_tts = FakeTTS_ValidWAV()
    real_router = EzzioRealtimeRouter()

    router_proc = EzzioRouterProcessor(router=real_router, name="router_r5")
    pre_collector = PreTTSCollector(name="collector_pre_tts")
    tts_proc = EzzioTTSProcessor(tts_engine=fake_tts, name="tts_r5")
    ...
    async def scenario():
        ...
        await worker.queue_frame(TextFrame("Un seul chunk de test pour le TTS."))
    ...
    if pre_collector.text_frames and post_collector.audio_frames:
        # Temps entre le TextFrame sortant du Router et le premier AudioFrame sortant du TTS
    ...
    print("\n" + "=" * 60)
    print(" V5.4a-R5 : " + ("PASS" if passed else "FAIL"))
    print("============================================================")
