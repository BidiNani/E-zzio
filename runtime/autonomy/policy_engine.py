...
class ActionPolicyEngine:
    ...
    def evaluate_action(self, intent_type: str, confidence: float) -> dict:
        ...
        if "PROTECTED" in intent_type or "SYSTEM" in intent_type or "ECOL" in intent_type:
            return {"autonomy_level": "HUMAN_ONLY", "action": "DENIED", "reason": "SYSTEM_INVARIANT_PROTECTION"}
        ...
        if not policy:
            ...
            return {"autonomy_level": "APPROVAL_REQUIRED", "action": "PROPOSAL_READY", "reason": "UNKNOWN_ACTION_DEFAULT_TO_APPROVAL"}
        ...
        if level == "AUTO_ALLOWED":
            ...
            if confidence >= req_conf:
                return {"autonomy_level": "AUTO_ALLOWED", "action": "AUTO_EXECUTE", "reason": "CONFIDENCE_THRESHOLD_MET"}
            else:
                return {"autonomy_level": "AUTO_ALLOWED", "action": "REQUEST_APPROVAL", "reason": "CONFIDENCE_BELOW_THRESHOLD"}

        elif level == "APPROVAL_REQUIRED":
            return {"autonomy_level": "APPROVAL_REQUIRED", "action": "PROPOSAL_READY", "reason": "POLICY_MANDATES_APPROVAL"}

        elif level == "HUMAN_ONLY":
            return {"autonomy_level": "HUMAN_ONLY", "action": "DENIED", "reason": "CRITICAL_ACTION_RESTRICTED"}

        return {"autonomy_level": "APPROVAL_REQUIRED", "action": "PROPOSAL_READY", "reason": "FALLBACK_RESTRICTION"}
