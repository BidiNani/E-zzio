    ...
    def load(self):
        if self.path.exists():
            try:
                ...
                self.messages = [Message.from_dict(m).to_dict() for m in raw_messages]
            except Exception:
                self.messages = []

    def save(self):
        payload = {"session_id": self.session_id, "updated_at": datetime.now().isoformat(), "messages": self.messages}
