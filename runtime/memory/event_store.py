...
class SimpleFileLock:
    ...
    def _log_security(self, event_type: str, details: dict):
        if not self.security_path:
            return
        sec_event = {"timestamp": time.time(), "type": event_type, "details": details, "version": "4.5.2"}
        try:
    ...
    def __enter__(self):
        ...
        while True:
            try:
                ...
                meta = {"pid": self.pid, "created": time.time(), "host": os.environ.get("COMPUTERNAME", "unknown"), "version": "4.5.2"}
    ...
    def _inspect_and_clear_if_stale(self) -> bool:
        try:
            ...
            if not _is_process_alive(lock_pid):
                self.lock_path.unlink(missing_ok=True)
                self._log_security(
                    "STALE_LOCK_RECOVERED", {"old_pid": lock_pid, "age_seconds": age, "reason": "dead_pid", "action": "removed"}
                )
                return True
            return False
...
class MemoryEventStore:
    ALLOWED_TYPES = {"SKILL_EXECUTED", "SKILL_FAILED", "SYSTEM_EVENT", "RECOVERY_EVENT", "SECURITY_EVENT"}
    GENESIS_HASH = "0" * 64

    def __init__(
        self,
        path="runtime/memory/events.jsonl",
        security_log_path="runtime/memory/security_events.jsonl",
        snapshot_path="runtime/memory/snapshot.json",
        archive_dir="runtime/memory/archive",
        quarantine_path="runtime/memory/events_quarantine.jsonl",
    ):
        self.path = Path(path)
    ...
    def _validate_on_boot(self):
        try:
            ...
            for idx, line in enumerate(lines):
                ...
                try:
                    ...
                    if idx == len(lines) - 1:
                        ...
                        self._log_security_event("MEMORY_CHAIN_COMPROMISED", {"status": "FAILED", "error": self.integrity_error})
                        return
            ...
            if valid:
                self.integrity_status = "VALID"
                self.integrity_error = None
                self._log_security_event("MEMORY_CHAIN_VALIDATION", {"status": "VALID", "checked_events": self.checked_events_count})
            else:
                self.integrity_status = "FAILED"
                self.integrity_error = msg
                self._log_security_event("MEMORY_CHAIN_COMPROMISED", {"status": "FAILED", "error": msg})
        except Exception as e:
            self.integrity_status = "FAILED"
            self.integrity_error = str(e)
            self._log_security_event("MEMORY_CHAIN_COMPROMISED", {"status": "FAILED", "error": str(e)})

    def assert_boot_integrity(self):
        if self.integrity_status != "VALID":
            raise RuntimeError(f"MEMORY INTEGRITY FAILURE: events.jsonl chain invalid. Details: {self.integrity_error}")
