        ...
        with self.store._lock:
            with sqlite3.connect(self.store.db_path) as conn:
                if ptype == "fact_candidate":
                    conn.execute(
                        "INSERT OR REPLACE INTO facts (fact_id, statement, confidence, origin_episodes, created_at, last_verified) VALUES (?, ?, ?, ?, ?, ?)",
                        (f"fact_{uuid.uuid4().hex[:8]}", prop["statement"], prop["confidence"], json.dumps(evidence), now, now),
                    )
                elif ptype == "rule_candidate":
                    conn.execute(
                        "INSERT OR REPLACE INTO rules (rule_id, condition, action, confidence, origin_episodes, is_active) VALUES (?, ?, ?, ?, ?, ?)",
                        (f"rule_{uuid.uuid4().hex[:8]}", "General", prop["statement"], prop["confidence"], json.dumps(evidence), 1),
                    )
                elif ptype == "belief_update":
                    conn.execute(
                        "INSERT OR REPLACE INTO beliefs (belief_id, statement, confidence, origin_episodes, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)",
                        (f"bel_{uuid.uuid4().hex[:8]}", prop["statement"], prop["confidence"], json.dumps(evidence), now, now),
                    )
                elif ptype == "skill_candidate":
                    conn.execute(
                        "INSERT OR REPLACE INTO skills (skill_id, name, description, trigger, procedure, confidence, origin_episodes, last_used) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                        (
