...
class ExecutionSandbox:
    ...
    def run_isolated_code(self, actor: str, capability: str, code_snippet: str, requested_budget: dict) -> dict:
        decision = self.context.policy.evaluate_intent(
            actor=actor, action=capability, target="execution.sandbox", context_permissions=[capability]
        )
        ...
        safe_env = {"PYTHONPATH": str(Path.cwd()), "EZZIO_WORKER": "true", "PATH": os.environ.get("PATH", "")}
        ...
        try:
            ...
            if returncode == 0:
                ...
                incident_payload = {"event": "WorkerCrashed", "pid": worker_pid, "exit_code": returncode, "status": "NON_ZERO_EXIT"}
                self.context.ledger.record(
                    execution_id=self.context.execution_id,
                    actor=actor,
                    capability=capability,
