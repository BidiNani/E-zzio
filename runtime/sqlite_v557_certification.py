...
class DummyTelemetry:
    def add_db_stats(self, *a, **k):
        pass

    def inc_retry(self, *a, **k):
        pass


CONFIG = {
    "paths": {"db_path": os.path.join(ROOT, "runtime", "test_sqlite_v557.db"), "backup_dir": os.path.join(ROOT, "runtime", "backups")},
    "sqlite_pragmas": {"busy_timeout_ms": 30000, "journal_size_limit_bytes": 67108864, "mmap_size_bytes": 268435456},
...
batch = []
for i in range(1000):
    batch.append((f"G:\\AI\\E-zzio\\file_{i}.py", f"file_{i}.py", "E-zzio", ".py", 1.5, time.time(), 1000 + i, b"hash_blob_123"))
...
def worker(offset):
    try:
        rows = [
            (f"G:\\AI\\E-zzio\\p_{offset + i}.py", f"p_{offset + i}.py", "E-zzio", ".py", 2.0, time.time(), 2000 + offset + i, b"hash")
            for i in range(100)
        ]
