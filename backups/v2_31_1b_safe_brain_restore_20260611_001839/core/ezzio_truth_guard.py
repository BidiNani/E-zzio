...
def correction_ack(text: str) -> str:
    ...
    if is_gaming_context(t):
        return (
            "Tu as raison. Je corrige l'interprétation : si tu indiques que c'est un logo vectoriel "
            "lié à World of Warcraft, je dois m'appuyer sur ce contexte au lieu de refuser.\n\n"
            "Interprétation corrigée : c'est un logo / emblème vectoriel de style fantasy lié au démoniste de WoW. "
            "Les formes anguleuses, les couleurs sombres/vives et le contraste fort évoquent bien une identité de classe magique, "
            "plutôt démoniaque ou occulte, pas un simple motif floral. Je ne dois pas prétendre qu'il s'agit d'une entreprise, "
            "ni proposer de PowerShell pour une image de logo."
        )
...
def sanitize_reply(reply: str, user_text: str = "") -> str:
    ...
    # Corrige les hallucinations matérielles.
    ...
    # Corrige les refus absurdes sur jeux vidéo / logos.
    if is_gaming_context(user) and any(refusal in low for refusal in BAD_REFUSALS):
        return (
            ...
            "Pour ton exemple : si tu dis que c'est un logo vectoriel du démoniste sur WoW, je dois utiliser ce contexte. "
            "Je peux alors décrire le style démoniste : formes agressives, énergie sombre, symbolique occulte/fel, "
            "contrastes marqués, silhouette d'emblème de classe. Je dois aussi préciser si je ne peux pas garantir "
            "que c'est un logo officiel plutôt qu'une version fan-made ou vectorielle."
        )

    # Si l'utilisateur corrige explicitement, ne pas contredire ni refuser.
