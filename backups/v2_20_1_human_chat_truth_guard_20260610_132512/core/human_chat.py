...
def list_sessions() -> Dict[str, Any]:
    ...
    return {
        "ok": True,
        "version": "v2.20-human-chat-memory",
...
def command_reply(text: str, session: str) -> Dict[str, Any] | None:
    ...
    if cmd in ("/help", "help"):
        return {
            "reply": (
                "Commandes E-ZZIO PC : /status, /maintenance, /human, /journal, /sessions, /brief. "
                "Sinon, parle normalement : je route vers le bon organe local."
            ),
            "command": "/help",
        }
...
Ne propose aucune action destructive sans confirmation explicite.
""".strip()
    ...
    try:
        ...
        reply = routed.get("reply") or "Je suis là, mais je n'ai pas produit de réponse exploitable."
        ...
        routed["version"] = "v2.20-human-chat-memory"
