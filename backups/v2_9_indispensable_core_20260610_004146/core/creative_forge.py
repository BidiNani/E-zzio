...
def status():
    return {
        "version": "v2.8.1-cpu-ram-only-creative-lock",
        "policy": {
            "global": "CPU/RAM only",
            "ollama_gpu": "disabled",
            "comfyui_gpu": "disabled by launch script --cpu",
        ...
        "practical_note": {
            "image_cpu": "possible mais très lent",
            "video_cpu": "préférer assemblage de frames / petites résolutions / durées courtes",
            "apk_cpu": "OK, build classique Android/Gradle",
        },
    }
...
def save_prompt_file(kind: str, prompt: str, negative: str = "", meta: Optional[dict] = None):
    ...
    payload = {
        ...
        "meta": meta or {},
        "cpu_ram_only": True,
        "note": "Prompt préparé par E-ZZIO. Génération réelle via pipeline CPU-only ou ComfyUI lancé avec --cpu.",
    }
...
def apk_status():
    ...
    return {
        ...
        "note": "Build APK = CPU classique. L'APK est une interface, pas le moteur Ollama complet.",
    }
