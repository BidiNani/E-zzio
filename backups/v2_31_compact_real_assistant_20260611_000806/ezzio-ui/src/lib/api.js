...
export async function apiUploadFirstAvailable(paths, formData, timeoutMs = 300000) {
  let last = null;
  ...
  for (const path of paths) {
    ...
    const result = await apiUpload(path, cloned, timeoutMs);

    if (result?.ok !== false && result?.status !== 404 && result?.error !== 'Not Found') {
      return {
        ...result,
        used_endpoint: path
      };
  ...
  return {
    ok: false,
    reply: 'Endpoint Vision introuvable. Aucun endpoint upload compatible n’a répondu.',
    error: 'vision_upload_endpoint_not_found',
    last
  };
}
