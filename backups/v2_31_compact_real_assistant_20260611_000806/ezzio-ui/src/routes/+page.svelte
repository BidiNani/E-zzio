  ...
  let messages = $state([
    {
      ...
      role: 'assistant',
      text:
        "Salut Enrik. Ecris ta demande ici, ou glisse une image dans la zone de chat. Je choisirai automatiquement entre chat, recherche, analyse image ou systeme. Pour un logo ou une icone, je decris les formes, les couleurs, le style et l'usage possible.",
      kind: 'welcome',
  ...
  async function runVision(prompt) {
    ...
    if (first?.file) {
      ...
      form.append(
        'prompt',
        prompt ||
          "Analyse l'image comme un logo, une icone, une illustration ou une capture selon son contenu. Decris precisement les formes, couleurs, contours, style et ce que cela evoque. Ne parle de PowerShell que si c'est une capture technique."
      );

      return await apiUploadFirstAvailable(['/vision/analyze-smart', '/api/vision/analyze-smart', '/vision/analyze', '/api/vision/analyze'], form, 300000);
    }
    ...
    <div class="composer-wrap">
      ...
      >
        ...
        <textarea
          bind:value={text}
          onkeydown={onKeydown}
          placeholder="Ecris a E-ZZIO... ou glisse une image ici. Exemple : decris ce logo precisement."
          disabled={busy}
        ></textarea>
