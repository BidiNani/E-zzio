...
def registry() -> Dict[str, Any]:
    return {
        "ok": True,
        "version": "v2.21-safe-action-queue",
...
def status() -> Dict[str, Any]:
    queue = read_jsonl(QUEUE_PATH, limit=200)
    runs = read_jsonl(RUNS_PATH, limit=50)
    pending = [item for item in queue if item.get("status") == "proposed"]
...
def queue(limit: int = 50) -> Dict[str, Any]:
    items = read_jsonl(QUEUE_PATH, limit=limit)
    return {
        "ok": True,
        "version": "v2.21-safe-action-queue",
        "count": len(items),
...
def find_proposal(proposal_id: str) -> Optional[Dict[str, Any]]:
    items = read_jsonl(QUEUE_PATH, limit=500)
