    ...
    def execute(self, request: ToolRequest, session_id: str) -> ToolResult:
        ...
        if not authorized or not token:
            ...
            return ToolResult(
                ...
                # Signer le token fraichement généré avant de le vérifier
        signature = self.signer.sign(token, self.key_manager.get_key())
        if not self.signer.verify(token, signature, self.key_manager.get_key()):
        ...
        try:
            ...
            self.event_bus.emit(
                "AuditLog",
                {
                    "message": f"Execution returned tool={request.name} success={(result.get("success", False) if isinstance(result, dict) else getattr(result, "success", False))}",
                    "level": "INFO"
                }
            )
            ...
            if res_id:
                if result and (result.get("success", False) if isinstance(result, dict) else getattr(result, "success", False)):
