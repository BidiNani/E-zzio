...
class AuditLogger:
    ...
    def log_tool(self, request, result, duration: float, metadata: dict = None):
        record = {
            ...
            "error": (result.get("error", "") if isinstance(result, dict) else getattr(result, "error", "")) if not (result.get("success", False) if isinstance(result, dict) else getattr(result, "success", False)) else None,
