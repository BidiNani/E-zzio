...
class RSSCollector:
    ...
    def parse_xml(self, xml_data: bytes) -> List[Dict[str, Any]]:
        ...
        for item in items:
            ...
            published_at = self._extract_text(item, 'pubDate') or self._extract_text(item, 'published') or self._extract_text(item, 'updated')
    ...
    def fetch_and_parse(self, feed_url: str) -> List[Dict[str, Any]]:
        try:
            req = urllib.request.Request(
                feed_url, 
                headers={'User-Agent': 'E-ZZIO-CognitiveAgent/2.5'}
            )
