...
class RSSCacheStore:
    ...
    def get_recent_articles(self, limit: int = 10) -> List[Dict[str, Any]]:
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute("SELECT guid, title, link, summary, published_at FROM articles ORDER BY ingested_at DESC LIMIT ?", (limit,))
            return [
                {"guid": row[0], "title": row[1], "link": row[2], "summary": row[3], "published_at": row[4]}
